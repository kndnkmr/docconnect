...
import { adminAPI, announcementAPI, complaintAPI, doctorAPI, availabilityAPI } from '../services/api';
...
function AdminDashboard() {
  ...
  const [bookDoctorResults, setBookDoctorResults] = useState([]);
  const [bookSelectedDoctor, setBookSelectedDoctor] = useState(null);
  const [bookSlots, setBookSlots] = useState([]);
  const [bookSlotsLoading, setBookSlotsLoading] = useState(false);
  const [bookSubmitting, setBookSubmitting] = useState(false);

  // Search doctors for the booking form (reuses the public doctor list).
  const handleBookDoctorSearch = async (e) => {
    e.preventDefault();
    if (!bookDoctorQuery.trim()) return;
    try {
      const res = await doctorAPI.getAll({ name: bookDoctorQuery.trim(), limit: 10 });
      setBookDoctorResults(res.data.doctors || []);
      if ((res.data.doctors || []).length === 0) toast('No doctors found for that name', { icon: '🔍' });
    } catch (error) {
      toast.error('Failed to search doctors');
    }
  };

  // Pick a doctor for the booking → reset date/slot selection.
  const handleBookSelectDoctor = (doc) => {
    setBookSelectedDoctor(doc);
    setBookForm((f) => ({ ...f, doctorId: doc._id, date: '', timeSlot: '' }));
    setBookSlots([]);
    setBookDoctorResults([]);
    setBookDoctorQuery('');
  };

  // When admin picks a date, fetch that doctor's free slots (same public
  // endpoint patients use — so the admin only ever sees genuinely open slots).
  const handleBookDateChange = async (date) => {
    setBookForm((f) => ({ ...f, date, timeSlot: '' }));
    setBookSlots([]);
    if (!bookForm.doctorId || !date) return;
    setBookSlotsLoading(true);
    try {
      const res = await availabilityAPI.getFreeSlots(bookForm.doctorId, date);
  ...
  const handleBookSubmit = async (e) => {
    e.preventDefault();
    const { patientName, patientPhone, doctorId, date, timeSlot, reason } = bookForm;
    if (!patientName || !patientPhone || !doctorId || !date || !timeSlot || !reason) {
      toast.error('Please fill patient name, phone, doctor, date, slot, and reason');
      return;
    }
    setBookSubmitting(true);
    try {
      const res = await adminAPI.bookForPatient(bookForm);
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex border-b mb-6 overflow-x-auto">
        ...
        <button
          ...
          className={`px-6 py-3 font-medium border-b-2 transition-colors whitespace-nowrap ${
            activeTab === 'book' ? 'border-primary-600 text-primary-600' : 'border-transparent text-gray-500 hover:text-gray-700'
          }`}
        >
          Book for Patient
        </button>
      ...
      {activeTab === 'book' && (
        <div className="max-w-2xl">
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6 text-sm text-blue-800">
            <strong>Book on behalf of a patient</strong> who called or messaged and can't book themselves.
            We'll find them by phone (or create a new patient account), then book a free slot with the doctor.
            The appointment is created as <em>pending</em> — the doctor confirms it the usual way.
          </div>

          <form onSubmit={handleBookSubmit} className="bg-white rounded-xl shadow-md p-6 space-y-5">
            {/* Patient details */}
            <div>
              <h3 className="font-semibold text-gray-800 mb-3">1. Patient details</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-sm text-gray-600 mb-1">Phone (10-digit) *</label>
                  <input
                    type="tel"
                    value={bookForm.patientPhone}
                    onChange={(e) => setBookForm((f) => ({ ...f, patientPhone: e.target.value }))}
                    className="w-full border border-gray-300 rounded-lg px-3 py-2"
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setBookForm((f) => ({ ...f, patientEmail: e.target.value }))}
                    className="w-full border border-gray-300 rounded-lg px-3 py-2"
            ...
            {/* Doctor selection */}
            <div>
              <h3 className="font-semibold text-gray-800 mb-3">2. Choose doctor</h3>
              {bookSelectedDoctor ? (
                <div className="flex items-center justify-between bg-primary-50 border border-primary-200 rounded-lg px-4 py-3">
                  <div>
                    <div className="font-medium text-gray-800">Dr. {bookSelectedDoctor.name}</div>
                    <div className="text-xs text-gray-500">{bookSelectedDoctor.specialization}{bookSelectedDoctor.city ? ` · ${bookSelectedDoctor.city}` : ''}</div>
                  </div>
                  <button
                    type="button"
                    onClick={() => { setBookSelectedDoctor(null); setBookForm((f) => ({ ...f, doctorId: '', date: '', timeSlot: '' })); setBookSlots([]); }}
                ...
                <div>
                  <div className="flex gap-2">
                    <input
                      type="text"
                      value={bookDoctorQuery}
                      onChange={(e) => setBookDoctorQuery(e.target.value)}
                      onKeyDown={(e) => { if (e.key === 'Enter') handleBookDoctorSearch(e); }}
                      className="flex-1 border border-gray-300 rounded-lg px-3 py-2"
                      placeholder="Search doctor by name"
                    />
                    <button
                      type="button"
                      onClick={handleBookDoctorSearch}
                      className="bg-primary-600 text-white px-4 py-2 rounded-lg font-medium hover:bg-primary-700"
                    >
                      Search
                    </button>
                  </div>
                  {bookDoctorResults.length > 0 && (
                    <div className="mt-2 border border-gray-200 rounded-lg divide-y max-h-56 overflow-y-auto">
                      {bookDoctorResults.map((doc) => (
                        <button
                          key={doc._id}
                          type="button"
                          onClick={() => handleBookSelectDoctor(doc)}
                          className="w-full text-left px-4 py-2 hover:bg-gray-50"
                        >
                          <div className="font-medium text-gray-800">Dr. {doc.name}</div>
                          <div className="text-xs text-gray-500">{doc.specialization}{doc.city ? ` · ${doc.city}` : ''} · ₹{doc.consultationFee}</div>
                        </button>
                      ))}
                    </div>
                  )}
            ...
            {bookSelectedDoctor && (
              <div>
                <h3 className="font-semibold text-gray-800 mb-3">3. Date &amp; time</h3>
                <label className="block text-sm text-gray-600 mb-1">Date *</label>
                <input
                  type="date"
                  value={bookForm.date}
                  min={new Date().toLocaleDateString('en-CA', { timeZone: 'Asia/Kolkata' })}
                  onChange={(e) => handleBookDateChange(e.target.value)}
                  className="w-full sm:w-auto border border-gray-300 rounded-lg px-3 py-2"
                />
                {bookForm.date && (
                  <div className="mt-3">
                    <label className="block text-sm text-gray-600 mb-1">Available slots *</label>
                    {bookSlotsLoading ? (
                      <p className="text-sm text-gray-500">Loading slots…</p>
                    ) : bookSlots.length === 0 ? (
                      <p className="text-sm text-gray-500">No free slots for this date. Try another day.</p>
                    ) : (
                      <div className="flex flex-wrap gap-2">
                        {bookSlots.map((slot) => (
                          <button
                            key={slot}
                            type="button"
                            onClick={() => setBookForm((f) => ({ ...f, timeSlot: slot }))}
            ...
            {/* Reason + type */}
            <div>
              <h3 className="font-semibold text-gray-800 mb-3">4. Consultation details</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
            ...
            <button
              ...
              className="w-full bg-primary-600 text-white py-3 rounded-lg font-semibold hover:bg-primary-700 disabled:opacity-60"
            >
