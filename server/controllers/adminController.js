...
const { sendAppointmentNotification } = require('../utils/sendEmail');
...
// ============================================
// BOOK APPOINTMENT ON BEHALF OF A PATIENT - Admin only
// ============================================
// Endpoint: POST /api/admin/appointments
// Body: { patientName, patientPhone, patientEmail?, doctorId, date, timeSlot,
//         reason, consultationType? }
//
// For patients who call/WhatsApp and can't book themselves. The admin
// provides the patient's phone + name; we find the existing patient by phone
// or create a minimal patient account, then book the slot using the SAME
// validations as a normal patient booking. The appointment lands as
// 'pending', so the doctor confirms it through the usual flow.
//
// Reuses (does not duplicate) the existing rules: doctor exists, slot free,
// not in the past. Marked bookedByAdmin for a clear audit trail.
...
const bookAppointmentForPatient = async (req, res) => {
  try {
    ...
    if (!patientName || !patientPhone || !doctorId || !date || !timeSlot || !reason) {
      return res.status(400).json({
        message: 'Please provide patientName, patientPhone, doctorId, date, timeSlot, and reason'
      });
    }

    // Step 2: Validate + format the patient's phone (same rules as signup)
    if (!isValidIndianPhone(patientPhone)) {
      return res.status(400).json({
        message: 'Please enter a valid 10-digit Indian mobile number for the patient'
      });
    }
    ...
    // Step 4: Check the date is not in the past (IST) — same check as booking
    ...
    // Step 5: Check the slot isn't already taken (same check as booking)
    ...
    // Step 6: Find an existing (non-deleted) patient by phone, or create one.
    let patient = await User.findOne({ phone: formattedPhone, role: 'patient', isDeleted: { $ne: true } });
    let createdNewPatient = false;

    if (!patient) {
      // Optional email — only use it if valid-looking and not already taken.
      const normalizedEmail = patientEmail ? patientEmail.toLowerCase().trim() : '';
      let emailToUse; // undefined unless we confirm it's free (sparse unique index)
      if (normalizedEmail) {
      ...
      // Assign a human-readable Patient ID (same scheme as signup)
      const seq = await getNextSequence('patientId');
      patient.patientId = `PT${String(seq).padStart(6, '0')}`;
      await patient.save({ validateBeforeSave: false });
      createdNewPatient = true;
    }

    // Step 7: Create the appointment (pending → doctor confirms as usual)
    const appointment = await Appointment.create({
      ...
      bookedFor: 'self',
      bookedByAdmin: true,
      // Consent recorded as admin-assisted (patient booked via phone/WhatsApp).
      consentGiven: true,
      consentAt: new Date(),
      consentIP: 'admin-assisted'
    });
    ...
    console.error('Admin book appointment error:', error.message);
    res.status(500).json({ message: 'Error booking appointment for patient' });
  }
};

module.exports = { getStats, getAllUsers, getAllAppointments, deleteUser, setUserSuspension, setDoctorVerification, getAnalytics, migrateBase64Images, generateResetLink, findDuplicatePhones, freeUpContactInfo, backfillPatientIds, backfillDoctorLanguages, sendDoctorSetupReminder, markEmailVerified, bookAppointmentForPatient };
